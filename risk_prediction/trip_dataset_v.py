    ...
    def prepare_input_sequence(self, batch, roi_bg=('BG_ZERO')): # <ADD roi_bg=('BG_ZERO')/>
        ...
        if self.feature_type == 'raw':
            for one_batch in batch:
                ...
                image_files = glob.glob(os.path.join(self.ds_path, one_batch[0], 'orig_img\\*'))
                roi_ffs = []
                for i in range(len(ffs)):
                    roi_ffs.append(self.extract_roi_feature(ffs[i], rbs[i], image_files[i], roi_bg)) # <ADD roi_bg/>
        ...
        return ffb_seq
    #
    def extract_roi_feature(self, feature,  box, image_file, roi_bg): # <ADD roi_bg/>
        """ Extract ROI feature
            Args:
        ...
        if bg == 'BG_ZERO':
            ...
            extracted_feature_mask = np.zeros_like(feature)
        elif bg == 'BG_GN':
            gn_mean = 0.0
            gn_std = roi_bg[1]
        ...
        for b in box: # [label, center-x, center-y, width, height] 
            ...
            extracted_feature_mask[:,:,l_y0:l_y1,l_x0:l_x1] = np.ones(feat_np[:,:,l_y0:l_y1,l_x0:l_x1].shape)
        feature_imgmask = cv2.resize(extracted_feature_mask[0][0].astype(np.uint8)*255, None, fx=24, fy=18)
        feature_img = cv2.resize(extracted_feature[0][0].astype(np.uint8)*255, None, fx=24, fy=18)

        cv2.imshow("ch0_feature_mask", feature_imgmask)
        cv2.imshow("ch0_feature", feature_img)
        cv2.imshow("orig_image", cv2.imread(image_file))
        cv2.waitKey(1)
